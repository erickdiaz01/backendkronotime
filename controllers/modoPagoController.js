...
const crearModoDePago= async (req, res = response) => {
  ...
  const getModoDePago = async (req, res = response) => {
    try {
      const modoDePago = await ModoDePago.findById(req.params.modoPagoId);
      res.status(200).json({
        ok: true,
  ...
  const editarModoDePago = async (req, res = response) => {
    try {
      const { modoPagoId, tipo } = req.body;
      let modoDePagoActualizado = await ModoDePago.findOneAndUpdate(
        { modoPagoId: req.params.modoPagoId },
        { tipo }
      );
  ...
  const eliminarModoDePago= async (req, res = response) => {
    try {
      let modoDePago = await ModoDePago.findById(req.params.modoPagoId);
      if (!modoDePago) {
        return res.status(400).json({
          ok: false,
